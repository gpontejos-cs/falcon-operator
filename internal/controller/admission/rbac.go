...
func (r *FalconAdmissionReconciler) reconcileServiceAccount(ctx context.Context, req ctrl.Request, log logr.Logger, falconAdmission *falconv1alpha1.FalconAdmission) (bool, error) {
	...
	// Preserve OpenShift-managed dockercfg and dockerconfigjson secrets
	if r.OpenShift {
		for _, existingSecret := range existingServiceAccount.ImagePullSecrets {
			// OpenShift automatically creates secrets with patterns:
			// - <serviceaccount-name>-dockercfg-<random> (legacy)
			// - <serviceaccount-name>-dockerconfigjson-<random> (modern)
			if strings.Contains(existingSecret.Name, "-dockercfg-") || strings.Contains(existingSecret.Name, "-dockerconfigjson-") {
