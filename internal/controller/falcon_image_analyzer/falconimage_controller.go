...
func (r *FalconImageAnalyzerReconciler) reconcileIARAgentService(ctx context.Context, req ctrl.Request, log logr.Logger, falconImageAnalyzer *falconv1alpha1.FalconImageAnalyzer) error {
	...
	// Ensure operator-managed labels exist with correct values, without removing OpenShift-added labels
	if existingService.ObjectMeta.Labels == nil {
		existingService.ObjectMeta.Labels = make(map[string]string)
	}
	for key, value := range service.ObjectMeta.Labels {
		if existingService.ObjectMeta.Labels[key] != value {
			existingService.ObjectMeta.Labels[key] = value
			updated = true
		}
	}
